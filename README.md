...
A page a running capsper serves about itself: what it is doing, what it has
recorded, and every setting behind it. Off until asked for.
...
It runs on its own account rather than as part of any capture mode, because a
capsper that only dictates has as much to show as one recording meetings. It
comes up before the model loads, so it answers during the minute of model
reading rather than being refused throughout it.

It binds to loopback, and the default is off, because this is the one part of
capsper that opens a socket onto what the machine is doing and what it has
recorded.

**Status**, at `/`: whether push-to-talk is held, how many remote clients are
connected, whether a meeting is being recorded and for how long. Which
microphone each capture settled on after following the desktop's default, the
level arriving on it and the gain auto-gain has chosen. Version, backend,
model and uptime.

**Transcripts**, at `/transcripts`: every meeting, newest first. Picking one
plays the audio with the transcript scrolling in step, near end on the left and
far end on the right. Clicking a line plays from there, and a control sends
either channel to both ears so the hard panning is a choice rather than
something to endure.

**Recordings**, at `/recordings`: the debug recordings, if `debug_recording.dir`
is set. The same player, without the channel control — an utterance is one
microphone, so there is no near and far to choose between.

### Changing the settings from it

`/settings` is the whole settings tree as a form, generated by walking the type
in `config.zig` with each field's own doc comment beside it. There is no list of
fields anywhere in the page: add a setting and it appears, documented, with its
enum's choices offered and the audio devices in the graph suggested on the
fields that name one.

Saving writes the file the settings came from and exits, so the service manager
starts capsper again on the new ones. A meeting being recorded at that moment is
closed properly first, so it survives as a finished file. Only the settings that
differ from their defaults are written, with each one's description above it —
the same output `--write-config` produces, for the same reason: a file naming
every default freezes today's into it.

On NixOS there is nothing to write. The settings are a read-only store path
built from your configuration, so saving changes nothing and says so, and hands
back the ZON to put where that file comes from. See [docs/nixos.md](docs/nixos.md).
...
Turn on [the console](#the-console) and the sessions are listed at
`/transcripts`, newest first, each playing against its transcript.
