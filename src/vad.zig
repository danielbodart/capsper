...
pub const VadFilter = struct {
    const threshold: f32 = 0.03; // low: detect quiet speech for onset
    const threshold_off: f32 = 0.01; // hysteresis: only un-trigger on definite silence
    const min_silence_bytes: usize = 16000; // 500ms at 32000 bytes/sec
...
pub const Vad = struct {
    ...
    pub fn init(model_path: [:0]const u8) !Vad {
        ...
        // Must be set before init — init also logs, but those messages are useful.
        // We suppress only during detect_speech calls by filtering in the callback.
...
test "processChunkProb: triggered, above threshold_off → resets silence counter" {
    ...
    try std.testing.expect(filter.processChunkProb(0.08));
    try std.testing.expect(filter.triggered);
    try std.testing.expectEqual(@as(usize, 0), filter.silence_bytes);
}
