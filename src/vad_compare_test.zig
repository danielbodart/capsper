...
/// Usage: vad-compare-test <wav_file>
...
pub fn main() !void {
    ...
    // Init both backends
    var ggml = try TenVadGgml.init(allocator, "dist/models/ten-vad-ggml.bin");
    defer ggml.deinit();

    var native = try TenVadNative.init();
    defer native.deinit();

    const chunk_bytes = TenVadGgml.chunk_bytes; // 512 = 256 samples * 2
    ...
    while (pos + chunk_bytes <= pcm.len) {
        ...
        const prob_ggml = ggml.chunkProbS16(chunk);
        const prob_native = native.chunkProbS16(chunk);
        const delta = @abs(prob_ggml - prob_native);

        if (delta > 0.001) {
            if (n_mismatch < 20) { // Print first 20 mismatches
    ...
    std.debug.print("{s}\n", .{"-" ** 44});
    std.debug.print("Frames: {d}, Mismatches (>0.001): {d}\n", .{ n_frames, n_mismatch });
