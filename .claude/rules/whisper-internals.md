...
- **Token accumulation**: Pipeline maintains `accumulated_tokens` across decode cycles within a VAD segment. After emitting words, confirmed tokens are committed as forced prefix for the next cycle. Reset on segment boundary, flush, or 28s buffer trim.
- **Incremental mel**: `MelBuffer` in `mel.zig` caches raw (pre-normalization) mel frames. Each cycle only computes FFT for new audio frames. `exportForWhisper` normalizes and zero-pads to 3000 frames for the encoder. Mel frame count is clamped to 3000 as a safety bound. Reset on segment boundary or 28s buffer trim.
- **Sliding window**: Audio buffer capped at 28s (`max_buffer_bytes=896000`), kept under whisper's 30s hard limit with a 2s safety margin. On trim, mel cache and accumulated tokens are reset (clean slate).
