...
- **Incremental mel**: `MelBuffer` in `mel.zig` caches raw (pre-normalization) mel frames. Each cycle only computes FFT for new audio frames. `exportForWhisper` normalizes and zero-pads to 3000 frames for the encoder. Reset on segment boundary or 15s buffer trim.
- **Sliding window**: Audio buffer capped at 15s (`max_buffer_bytes=480000`). On trim, mel cache and accumulated tokens are reset (clean slate).
