...
- **Linux dist builds target `x86_64_v3`** (AVX2+FMA+BMI) — inference runs on the CPU, so the vector instructions are the point, and everything from ~2015 onwards has them. Both `build()` and `ci()` pass `-Dcpu=x86_64_v3` to zig build.
